...
|---|---|---|---|
| BookSim2 selected timing | Git commit <code>28f43299f1706a3160ffac721ca461d74eb6e618</code>, native binary SHA-256 <code>7579da3acfd960ad02c18ca9257b87dfb617f8916246207749dff8ff4fee5c20</code>, 4x4 mesh, tested 1/3/4-flit packets, 1/2 VCs, depths 1/4/16, dimension-order plus tested XY/YX and ROMM choices, Bernoulli uniform traffic at rate 0.02 with seed 40 | Managed <code>AspdacStageNocRuntime</code>, six live tests, four frozen contract summaries, nine event categories, and eleven repeat-hash rows | Event-exact only for the selected tested contract. Figure 5 uses the separate <code>AspdacTransportRuntime</code> kernel |
| BookSim2 saturation study | Git commit <code>28f43299f1706a3160ffac721ca461d74eb6e618</code>, binary SHA-256 <code>44b617ec81bcdb7496ee86acab011f5d7d00c0716ae65150a11777a2b84c4cbd</code>, 4x4 mesh, dimension-order routing, one 128-bit flit per packet, one VC of 16 flits, four traffic patterns, and 10 seeds | <code>v_bs.yaml</code>, <code>rq2_booksim_stage.yaml</code>, <code>generate_booksim_configs.py</code>, and a locked ledger for 520 registered plus 10 uniform-extension configurations | Congestion ordering only. Absolute hotspot saturation is 0.04 versus 0.08 and is not called equivalent |
...
## BookSim selected-contract timing replay

The portable frozen check is <code>python scripts/validate_frozen.py --claim booksim_timing</code>. It verifies the paper-facing native comparisons and repeat hashes. The <code>paper</code> test group also executes six live managed STAGE contracts without invoking a BookSim process or backend. A fresh native BookSim2 execution remains optional and separate from the portable check.

The selected-contract evidence does not relabel submitted Figure 5. That figure is derived from the lightweight cycle-stepped <code>AspdacTransportRuntime</code> study, while the BookSim-matched production kernel is <code>AspdacStageNocRuntime</code>.
